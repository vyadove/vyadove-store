...
export const deleteMedia: CollectionAfterDeleteHook<Product> = async ({ doc, req }) => {
  const { payload } = req
    ...
    for (const image of variant.gallery) {
      if (isExpandedDoc<Media>(image)) {
        if (!mediaMap.has(image.filename!)) {
          ...
          req.payload.logger.debug(`Duplicate filename detected, skipping: ${image.filename}`)
        }
      } else {
  ...
  const deletionResults = await Promise.allSettled(
    uniqueImages.map((image) =>
      payload
        ...
        .then(() => {
          payload.logger.debug(`Successfully deleted: ${image.filename}`)
        })
        .catch((error) => {
          payload.logger.error(`Failed to delete: ${image.filename}`, error)
        }),
    ),
  )
