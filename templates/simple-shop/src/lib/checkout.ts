...
import type { Payment, Shipping, Order } from "@shopnex/types";
        ...
        return response.docs.map((shipping) => {
            ...
            return {
                ...
                freeShippingMinOrder:
                    provider?.freeShippingMinOrder ?? undefined,
                estimatedDeliveryDays:
                    provider?.estimatedDeliveryDays ?? undefined,
...
export async function createOrder(orderData: {
    ...
    try {
        ...
        const url = `${process.env.NEXT_PUBLIC_SERVER_URL}/api/orders/simple-checkout`;
        console.log("Making checkout request to:", url);
        console.log("Request body:", requestBody);

        // Use the simple-checkout endpoint instead of direct order creation
...
async function getShopHandle(): Promise<string> {
    if (typeof window !== "undefined") {
        ...
        if (
            parts.length >= 2 &&
            !["www", "app", "api", "localhost"].includes(parts[0])
        ) {
            return parts[0];
        }
    }
...
export function calculateShipping(
    ...
    if (
        shippingMethod.freeShippingMinOrder &&
        subtotal >= shippingMethod.freeShippingMinOrder
    ) {
        return 0;
    }
