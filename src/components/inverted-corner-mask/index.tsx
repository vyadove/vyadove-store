...
const InvertedCornerMask = (props: PropsWithChildren<Props>) => {
	...
	useEffect(() => {
		...
		const invertedCornerKey = (Object.keys(props.invertedCorners) as CornerKey[])
			.filter((key) => props.invertedCorners[key as keyof InvertedCorners]?.inverted)
			.map((key) => {
				return {
					[key]: {
						...
						roundedContent: props.invertedCorners[key as keyof InvertedCorners]?.roundedContent,
						corners: [props.cornersRadius, props.cornersRadius, props.cornersRadius],
						inverted: true,
					},
				};
			});
	...
	return (
		<div className={twMerge("relative", props.containerProps?.className)} ref={containerRef}>
			<div
				className={twMerge(
					"absolute z-20",
					roundedCorner === "tr" && `top-0 right-0 rounded-bl-[${props.cornersRadius}px]`,
					roundedCorner === "tl" && `top-0 left-0 rounded-br-[${props.cornersRadius}px]`,
					roundedCorner === "br" && `right-0 bottom-0 rounded-tl-[${props.cornersRadius}px]`,
					roundedCorner === "bl" && `bottom-0 left-0 rounded-tr-[${props.cornersRadius}px]`,
				)}
			...
			>
				<defs>
					<linearGradient
						...
						y2="100%"
					>
						<stop offset="0%" stop-color="#EAE2D6" stop-opacity="1"></stop>

						<stop offset="0%" stop-color="#EAE2D6" stop-opacity="1"></stop>

						<stop offset="100%" stop-color="#A0BAB7" stop-opacity="1"></stop>
					</linearGradient>
					...
					{/* Clip path built from the dynamic shape */}
					{/*{imageMaskSrc && (
            <clipPath id={clipId}>
              <path d={pathCode} />
            </clipPath>
