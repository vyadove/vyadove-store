...
import { gcd, generatePath } from "@/components/inverted-corner-mask/utils";
...
const InvertedCornerMask = (props: PropsWithChildren<Props>) => {
  ...
  useEffect(() => {
    ...
    const invertedCornerKey = (Object.keys(props.invertedCorners) as CornerKey[]).filter(
      (key) => props.invertedCorners[key as keyof InvertedCorners]?.inverted,
    ).map(
      (key) => {
  ...
  return (
    ...
    >
      ...
      {/*  --- SVG MASK --- */}
      <svg
        className="absolute -z-10 h-full w-full overflow-visible hidden"
        height="100%"
        id="preview"
