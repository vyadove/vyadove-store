...
export class Main {
    ...
    async init(): Promise<void> {
        try {
            ...
            console.log("\n");
            p.intro(chalk.bgCyan(chalk.black(" create-shopnex-app ")));
            p.note("Welcome to ShopNex. Let's create your e-commerce store!");
            ...
            if (nextConfigPath) {
                this.args["--name"] = slugify(
                    path.basename(path.dirname(nextConfigPath))
                );
            }
            ...
            if (nextConfigPath) {
                p.log.step(
                    chalk.bold(
                        `${chalk.bgBlack(` ${figures.triangleUp} Next.js `)} project detected!`
                    )
                );
                ...
                const proceed = await p.confirm({
                    initialValue: true,
                    message: chalk.bold(
                        `Install ${chalk.green("Payload")} in this project?`
                    ),
                });
                ...
                info("ShopNex e-commerce store successfully initialized!");
                p.note(
                    successfulNextInit(),
                    chalk.bgGreen(chalk.black(" Documentation "))
                );
                p.outro(feedbackOutro());
                return;
            }
            ...
            if (debugFlag) {
                debug(
                    `Using ${exampleArg ? "examples" : "templates"} from git tag: v${LATEST_VERSION}`
                );
            }
            ...
            if (!exampleArg) {
                ...
                switch (template.type) {
                    ...
                    case "starter": {
                        const dbDetails = await selectDb(
                            this.args,
                            projectName
                        );
            ...
            info("ShopNex e-commerce store successfully created!");
