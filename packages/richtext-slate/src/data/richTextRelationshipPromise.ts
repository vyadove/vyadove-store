        ...
        (children as any[]).forEach((element) => {
            if (
                        ...
                        populate({
                            ...
                            select:
                                req.payloadAPI !== "GraphQL"
                                    ? (populateArg?.[collection.config.slug] ??
                                      collection.config.defaultPopulate)
                                    : undefined,
                            showHiddenFields,
                        })
                    );
                ...
                if (
                    element.type === "upload" &&
                    Array.isArray(
                        field.admin?.upload?.collections?.[element?.relationTo]
                            ?.fields
                    )
                ) {
                    recurseNestedFields({
                        ...
                        depth,
                        draft,
                        fields: field.admin.upload.collections[
                            element.relationTo
                        ].fields,
                        overrideAccess,
                        populateArg,
            ...
            if (element.type === "link") {
                if (element?.doc?.value && element?.doc?.relationTo) {
                    const collection =
                        req.payload.collections[element?.doc?.relationTo];
