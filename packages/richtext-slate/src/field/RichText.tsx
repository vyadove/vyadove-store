...
import type { LoadedSlateFieldProps } from "./types";
...
const RichTextField: React.FC<LoadedSlateFieldProps> = (props) => {
    ...
    const renderLeaf = useCallback(
        ({ attributes, children, leaf }) => {
            const matchedLeaves = Object.entries(leaves).filter(
                ...
                >
                    <div className={`${baseClass}__wrapper`}>
                        {Object.keys(elements)?.length +
                            Object.keys(leaves)?.length >
                            0 && (
                            <div
                                className={[
                                    `${baseClass}__toolbar`,
                                    drawerIsOpen &&
                                        `${baseClass}__drawerIsOpen`,
                                ]
                                    .filter(Boolean)
                                    .join(" ")}
                                ref={toolbarRef}
