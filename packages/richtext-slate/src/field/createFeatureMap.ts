...
export const createFeatureMap = (
    ...
    for (const [key, value] of richTextComponentMap) {
        ...
        if (
            key.startsWith("leaf.button") ||
            key.startsWith("leaf.component.")
        ) {
            const leafName = key
                .replace("leaf.button.", "")
                .replace("leaf.component.", "");
        ...
        if (
            key.startsWith("element.button.") ||
            key.startsWith("element.component.")
        ) {
            const elementName = key
                .replace("element.button.", "")
                .replace("element.component.", "");
        ...
        if (
            key.startsWith("leaf.plugin.") ||
            key.startsWith("element.plugin.")
        ) {
