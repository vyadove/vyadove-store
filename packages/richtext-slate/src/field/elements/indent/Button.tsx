...
import { IndentLeft } from "../../icons/IndentLeft/index";
import { IndentRight } from "../../icons/IndentRight/index";
...
export const IndentButton: React.FC = () => {
            ...
            if (dir === "left") {
                if (isElementActive(editor, "li")) {
                    const [, listPath] = getCommonBlock(
                        editor,
                        (n) =>
                            Element.isElement(n) && listTypes.includes(n.type)
                    );

                    const matchedParentList = Editor.above(editor, {
                        at: listPath,
                        match: (n: ElementNode) =>
                            !Editor.isEditor(n) && isBlockElement(editor, n),
                    });

                    if (matchedParentList) {
                        const [parentListItem, parentListItemPath] =
                            matchedParentList;
                        ...
                        if (parentListItem.children.length > 1) {
                            ...
                            const [newParentNode] = Editor.node(
                                editor,
                                parentListItemPath
                            );
                            ...
                            if (
                                Element.isElement(newParentNode) &&
                                newParentNode.type === "li"
                            ) {
                                // Lift the nested lis
                    ...
                    Transforms.unwrapNodes(editor, {
                        match: (n) =>
                            Element.isElement(n) && n.type === indentType,
                        mode: "lowest",
                        split: true,
                    });
                }
            ...
            if (dir === "right") {
                ...
                if (isCurrentlyOL || isCurrentlyUL) {
                    ...
                    const [[, firstSelectedLIPath]] = Array.from(
                        Editor.nodes(editor, {
                            match: (node) =>
                                Element.isElement(node) && node.type === "li",
                            mode: "lowest",
                        })
                    );

                    // Is the first selected li the first in its list?
                    const hasPrecedingLI =
                        firstSelectedLIPath[firstSelectedLIPath.length - 1] > 0;
                    ...
                    if (hasPrecedingLI) {
                        ...
                        const precedingLIChildrenIsText =
                            Text.isText(precedingLIChildren);
    ...
    const canDeIndent =
        isElementActive(editor, "li") || isElementActive(editor, indentType);
        ...
        <React.Fragment>
            <button
                className={[baseClass, !canDeIndent && `${baseClass}--disabled`]
                    ...
                    .join(" ")}
                onClick={
                    canDeIndent ? (e) => handleIndent(e, "left") : undefined
                }
                type="button"
            >
                <IndentLeft />
