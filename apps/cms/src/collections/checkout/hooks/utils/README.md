...
```typescript
import { calculateCheckoutPricing, applyPricingToCheckout } from './utils/calculate-pricing';

export const myHook: CollectionBeforeChangeHook<Checkout> = async ({ data, req }) => {
  // Calculate pricing
...
export const myEndpoint: Endpoint = {
  ...
  path: '/calculate',
  handler: async (req) => {
    const pricing = await calculateCheckoutPricing(req.payload, checkoutData);
...
test('calculates pricing correctly', async () => {
  ...
  const checkoutData = {
    items: [
      { variantId: 'v1', product: 1, quantity: 2, unitPrice: 10 }
    ]
  };
