...
import { TypographyH1, TypographyH3, TypographyH6, TypographyMuted } from "@ui/shadcn/typography";
...
const ProductPreview = (
	...
	return (
		<Link className="group/card flex flex-col rounded-xl p-0 " href={`/products/${product.handle}`}>
			<div className="">
				{/* --- THUMBNAIL --- */}
				<InvertedCornerMask
					...
					cornerContent={
						<div className="flex items-center justify-center gap-2 p-2">
							<Button className="bg-accent/40 text-black group-hover/card:-rotate-45 " size="icon-lg">
								<AiOutlineArrowRight className='' />
							</Button>
						</div>
					}
			...
			<div className="flex flex-1 flex-col gap-2 rounded-xl p-4 shadow-sm">
				...
				<div className="itmes-center mt-auto flex gap-2">
					...
					<ItemDescription className="flex-1 text-wrap">{product.description}</ItemDescription>
				</div>
				...
				<div className='mt-auto flex items-center gap-4'>
					<TypographyH6 className="text-muted-foreground mt-auto">ETB {price?.toLocaleString("en-US")}</TypographyH6>
					{
						originalPrice && (
							<TypographyH6 className="text-muted-foreground/60 font-normal line-through">ETB {originalPrice?.toLocaleString("en-US")}</TypographyH6>
						)
					}
				</div>

				<Button className='bg-accent/60 mt-2 self-start' >
					Add to Cart
				</Button>
			</div>
		</Link>
