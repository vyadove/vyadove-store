...
export async function Collections() {
	...
	const collections = await payloadSdk.find({
		...
		limit: 6,
		select: { id: true, title: true, description: true, imageUrl: true, handle: true }, // fetch only needed fields
		...
		<div className="mt-24 flex flex-col gap-8">
			<div>
				...
				<TypographyP className="text-muted-foreground max-w-xl">
					Explore our diverse range of categories to find the perfect experience that suits your interests and
					desires.
				</TypographyP>
			</div>
			...
			<div className="grid gap-6 sm:grid-cols-2 md:grid-cols-3 lg:gap-8">
				{collections.docs.map((collection) => (
					<div className="group flex flex-col rounded-xl p-0" key={collection.id}>
						<InvertedCornerMask
							className="w-full"
							cornerContent={
						...
						<div className="flex flex-1 flex-col gap-4 p-4">
							<TypographyH3 className="line-clamp-2">{collection.title}</TypographyH3>
						</div>
					</div>
				))}
			</div>
