...
import { TypographyH2, TypographyLarge, TypographyLead, TypographyMuted } from "@ui/shadcn/typography";
...
import companyIcons from "@/scenes/testimonial/list";
...
const testimonials = [
	...
	},
	{
		name: "Jane Smith",
		role: "Freelancer",
		quote: "I love the variety of experiences available on Vyadove. There's something for everyone!",
		image: Person3,
	},
	{
		name: "Michael Brown",
		role: "Product Manager, Tech Solutions",
		quote: "The customer service at Vyadove is top-notch. They really care about their customers.",
		image: Person4,
	}
]
		...
		<InvertedCornerMask
			...
			cornerContent={
				<div className="flex items-center justify-center gap-2 p-3 sm:p-6">
					<Button className="bg-accent/20 cursor-pointer text-black md:size-14" onClick={() => {
		...
		>
			<div className="flex h-full w-full flex-col gap-4 p-24 px-4">
				<section className="relative mx-auto flex w-full max-w-6xl flex-col items-center text-center">
					...
					<Swiper autoplay={{
						delay: 2000,
					}} className="mySwiper max-w-full p-4 md:max-w-4xl" effect={'fade'} grabCursor loop modules={[EffectFade]}  ref={swiperRef}>
									...
									<div className="bg-accent-foreground flex flex-col items-center text-center">
										...
										<div className="flex flex-col items-center">
											<div className="size-24 overflow-hidden rounded-full shadow ring-2 ring-white">
												<Image alt={customer.name} className="h-full w-full object-cover" src={customer.image} />
											</div>
											<div className="mt-2">
					...
					{/* Helper line */}
					<TypographyLead className="mt-5 sm:mt-10">Trusted by over {companyIcons.length}+ companies worldwide</TypographyLead>
						...
						>
							{companyIcons.map((l, idx) => (
								<li className="flex items-center gap-2 border pl-8" key={idx}>
									...
									<div className={`bg-[url( relative h-[20px] w-[80px]${l})]`}
									/>
								</li>
							))}
						</Marquee>
